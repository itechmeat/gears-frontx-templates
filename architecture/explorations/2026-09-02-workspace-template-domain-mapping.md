...
template-workspace-contacts/        features/contacts/
template-workspace-dashboard/       features/dashboard/
template-workspace-chat/            features/chat/
template-workspace-mail/            features/mail/
```

`features/` sits at the project root, not under the shell's `src/`, because the shell claims `src/` and a screen claiming a path nested inside that claim collides with it under the assembly conflict check. Four different templates write into the same `features/` parent, each owning exactly one child directory, and a wholesale claim of the parent by any one of them would collide with the other three the moment a second screen template is applied. Each screen template's exclusive subtree is therefore its own child directory only, never the parent - the ownership-boundary decision's own principle ("claim what you ship, not the whole parent dir") applied across four claimants of one parent. The directory name is illustrative; what holds is one child per screen, outside the shell's claim. Every screen manifest below reflects that.

A screen's feature carries no MFE package. For the composed application, the project wraps each screen with `template-mfe`, and the wrapping step writes the screen's MFE package - its `mfe.json`, its Module Federation `vite.config.ts`, `package.json` and `tsconfig.json`, following the `demo-mfe` package shape - where `template-mfe` keeps its MFE packages (`src-app/mfe_packages/` in `template-shell`'s layout), with the MFE package importing the feature it wraps. `template-mfe`'s ownership boundary claims its own named packages there, one child directory each (confirmed against `template-mfe/frontx-template.json` in this repository), not the packages its add-mfe-package skill scaffolds later. Whether that skill serves a project `template-workspace` establishes, unchanged, is open (Open Question 6).
...
- The `.frontx/ai/@gears-frontx/frontx-template-workspace/` bundle. It carries the shell's own AI guidelines and skills for working with and evolving the shell, and no add-a-screen skill: the retired `add-inbox-screen` skill has no counterpart in the shell. Every screen template may ship its own bundle the same way, under its own `.frontx/ai/` subtree (each screen's draft ownership boundary below claims one), with guidance for evolving that screen's feature and the presentation values its wrapped screen declares, and wrapping a screen into a microfrontend is `template-mfe`'s own add-mfe-package skill. What having no add-a-screen skill in the shell costs is worth stating exactly, because it is conditional rather than free. The scaffolding feature in the ecosystem repository, as amended, routes an add-a-unit request into the same scaffolding entry point a create-a-project request goes to, and that entry point offers the named unit to the installed inventory before treating it as per-unit work. Where a distinct installed template's own description claims the unit, selection answers with that template and the unit is delivered by one more `frontx add` - which is the case this family is built for, since each of the four screens is its own template claiming its own screen. Where no installed template's description claims it, the unit stays per-unit work attributed to whatever template's ground it falls in, to be realized by that template's own extension skills, and only when that ground belongs to a selected or already-applied template; a unit outside such ground is reported as residual work and nothing is written. The shell's bundle declares none, so such a unit is reported back as residual work naming the ground it falls in, with nothing written into that ground. Concretely: a fifth screen nobody has published a template for cannot be added to a workspace project by any automated path, and the developer is told so rather than having something improvised into the shell's territory. That is the intended trade - a screen is a template, not a skill run - and it holds only for as long as every screen the family means to offer has a template of its own.
...
**Dependencies that must appear and do not exist in `template-inbox` today** - this is the one place the mapping is not a file move: `template-inbox` has zero Module Federation surface by design (its own `vite.config.ts` says so). The shell must additionally take on the MF-host runtime: `@gears-frontx/mfes`, `@gears-frontx/gts-plugin`, and a build-time MF/GTS plugin and framework layer. That layer is published today only from `template-shell`'s own territory in this repository, as template-level packages, and a template-level package is used by its own template only, so the workspace shell cannot depend on it. It either carries the layer as its own template content or takes its framework-level parts from a library of the ecosystem repository (Open Question 1, not resolved here).
...
**Source that moves in:** `src/screens/contacts/` (8 files, 755 lines) into `features/contacts/`, plus the presentation values its wrapped screen declares (see "New GTS surface"). No `mfe.json` and no Module Federation build: those belong to the MFE package `template-mfe` writes when it wraps the screen (see "Territory recap").
...
- No MF-host-contract layer in the feature itself. The MFE package that wraps it takes the dependencies of `template-mfe`'s package shape, which in `demo-mfe`'s own production `package.json` on `feb342d1` are `@gears-frontx/react` (published from `template-shell/packages/react` in this repository, not from the ecosystem repository), `@tanstack/react-query`, and `@gears-frontx/frontx-template-shell`'s build export. The two `template-shell` packages are not available to a family member, since a template-level package is used by its own template only (PRD §5.2, `cpt-frontx-workspace-templates-fr-no-cross-member-import`); this is the known conflict carried into Open Question 1.
...
- No MF-host-contract layer in the feature; the wrapping MFE package's dependencies are as for contacts.
...
**Source that moves in:** `src/screens/inbox/` (11 files, 2310 lines) into `features/chat/`, with every identifier renamed `inbox` to `chat` (the directory, code, and route names diverge from the URL today - `chat` in the URL, `inbox` in the code - and the rename closes that); the chat slice of `api/{dataset,mocks,types}.ts` and `InboxApiService.ts` (channels, conversations, messages); and `public/message-assets/{preview-chart,preview-diagram}.svg`, moved into chat's own assets and referenced relative to the feature rather than from the document root.
...
**Future-drift guard.** Duplicating a rule body across four independently-versioned files is safe only for as long as the four copies stay identical; nothing prevents one screen template's own future edit from silently diverging from another's copy of the same shared-group rule. The mitigation named here, to be picked up when the shared CSS is actually authored rather than only mapped: a parity check comparing each duplicated rule body's declared value set across the four screen templates' own module CSS files, failing when a shared-group class's rule text diverges between two members that both carry it. This mapping does not implement that check; it names it as the expectation the split's authors carry forward.

**i18n key split.** Two different strings, two different mechanisms. The screen's own internal UI copy follows the `useScreenTranslations` precedent already working in `_blank-mfe` (`import.meta.glob('./i18n/*.json')`, driven purely by the `language` shared property, no upward registration) - a self-contained namespace inside the screen's own bundle, needing nothing from the shell beyond the shared property it already receives. The one string that cannot follow that precedent is the menu label the shell itself renders as chrome (`presentation.label`): the shell cannot read a key out of a bundle it does not import, and on the load that paints the menu it has not loaded any unrouted screen's bundle at all. Admission is manifest-driven (`registerMfePackage` in `template-shell/src-app/app/mfe/bootstrap.ts`) and `Menu.tsx` builds the menu from `getExtensionsForDomain` without loading a remote, so a screen that has not been routed has executed no code and can dispatch nothing. The label therefore has to be static manifest data: a per-language map declared on the extension entry beside `label`, `icon`, `route` and `order`, validated by GTS at admission and read by the shell in the same pass. Prop-drilling `t`, or any dispatch-based hand-off, fails for the same reason - both need the screen's code to have run. Confidence: substantiated for both halves; the internal-copy half has a working precedent, and the label half rests on the two files named above rather than on a proposed mechanism.
...
**`public/message-assets/`.** Moves to chat's own assets, referenced relative to chat's feature rather than from the document root, and served from the wrapped screen's own MF publicPath in the wrapped case - already decided in the split plan, restated here because it is the one asset in the tree addressed by an absolute document-root path today (`dataset.ts` references `/message-assets/preview-chart.svg` and its companion), which is exactly the kind of reference that silently breaks once the document root is the shell's and the asset is chat's.

## New GTS surface

**Each wrapped screen carries one extension entry against the shell's existing screen domain** (`gts.frontx.mfes.ext.domain.v1~frontx.screensets.layout.screen.v1`, the same identifier `demo-mfe`'s Hello World extension already targets - no new domain is declared by any screen). The wrapping step declares it from the values the screen template states; the screen template itself carries no `mfe.json`. The concrete shape, following `demo-mfe/mfe.json`'s working pattern:
...
Four points on this shape carry over as settled, not re-opened here: `label` is a plain display string the shell renders as it stands - the shape the screen extensions in this repository already use - with `labels` beside it carrying the same string per language, so the menu localizes without the shell importing the screen's own translations and without the screen's code having run; `route` is now consumed by the shell for URL parsing and deep-link dispatch (`#/contacts/{id}` needs this - `template-shell` today declares `route` as schema-required but never reads it), and the family's own route set must stay prefix-free across members, the same way `/mail` and `/mailbox` applied together would not; `order` is banded per screen in inclusive, 100-wide ranges - contacts 100-199, dashboard 200-299, chat 300-399, mail 400-499 - with the 500-599 band left for a future fifth screen; `entries[].actions` is empty in this shape and stays empty - the family declares no action of its own, so nothing goes in it beyond what a screen otherwise needs. The Iconify-string convention (`icon: "lucide:users"`) is the shell's existing consumption contract (`@iconify/react` resolves the string), which diverges from how every screen renders its own internal icons today (`lucide-react` components imported directly) - the menu icon and a screen's internal icons are two different conventions living side by side by design, not an inconsistency to fix.
...
- URL routing of any kind. `template-shell` has none today (no `location.hash`, no `pushState`, no router anywhere in it, and nothing that reads `presentation.route`) - this is net-new code for the workspace shell, not a migration, driven by `presentation.route` prefix matching against the registered extension set.
- One new optional field, `presentation.labels`, on its own copy of the derived screen-extension schema (`template-shell/src/gts/schemas/extension_screen.v1.json`), carrying the per-language menu label beside the `label` that schema already requires. This is a schema edit in the shell's own template territory, not a new action and not a new channel.
...
**A dependency on another template's package, which no family member takes, and the known conflict next to it.** `demo-mfe` and the other MFE packages of `template-mfe` depend in production on `@gears-frontx/frontx-template-shell`'s published build export and on `@gears-frontx/react`, both published from `template-shell`'s own territory rather than from the ecosystem repository's `packages/`. A template-level package is used by the template that publishes it and by no other, so neither the workspace shell nor a screen template takes that dependency (PRD §5.2), while a wrapped screen that follows `template-mfe`'s package shape as it stands inherits it. The same dependency also sits outside the pin-drift guard: the guard walks every directory carrying a manifest and compares its ecosystem-package pins against the packages published from the ecosystem repository, so a drift on a template-published package is not caught by the mechanism that catches every other pin. Both are carried into Open Question 1.
...
1. **Where does the shell's MF-host build layer come from?** Not from `template-shell`'s published build export or its other packages: a template-level package is used by its own template only. Two options remain, and this mapping does not choose: `template-workspace` carries the layer as its own template content, copied with the rest of the template, or the framework-level parts of it come from a library of the ecosystem repository and the rest stays in the template. Which parts are framework-level is for the maintainers to settle. A known conflict bears on the answer: `template-mfe`'s MFE packages depend on `@gears-frontx/react` and `@gears-frontx/frontx-template-shell`, both published from `template-shell`, and a wrapped screen following that package shape inherits both (see Risks).
...
6. **Where does a screen template state its presentation values, and does `template-mfe` wrap it into a workspace project unchanged?** The values its wrapped screen declares (route, order, icon, `label`, `labels`) could live in the screen's own AI guidelines or skills, in a data file the wrapping step reads, or in a ready wrapper the screen ships beside its feature. `template-mfe`'s add-mfe-package skill targets `template-shell`'s layout (`src-app/`) today, and whether it serves a project `template-workspace` establishes is not verified.
7. **Is `template-workspace-chat` also the chat feature template a console outside this family composes, and does it keep the family prefix?** Nothing settled records that the two are one template; whether they are, and whether the member keeps the prefix or is named for its feature alone, as `template-chat`, is for the maintainers to settle; the same applies to any other screen once it is offered outside the family.
