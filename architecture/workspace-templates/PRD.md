...
#### No cross-sibling build-time import

- [ ] `p1` - **ID**: `cpt-frontx-workspace-templates-fr-no-cross-sibling-import`

A sibling's own source **MUST NOT** import another sibling's source, another sibling's published package, or the shell's own application code at build time. Each of the five siblings **MUST** carry its own thin API-glue layer, authored against `@gears-frontx/api`, for every endpoint it reads. Data whose subject matter belongs to another sibling **MUST** be reached over the shell's own REST surface at runtime, never through a shared module graph.

**Rationale**: Each sibling is bundled and versioned independently, and Module Federation gives each remote its own module graph, so a build-time import across two siblings either fails to resolve or silently pins one sibling's release to another's - the coupling the split exists to remove. The cost this buys is real and is priced here rather than rediscovered per sibling: every screen writes its own small equivalent of the shell's `registry.ts`/`queries.ts` pattern instead of importing it.
